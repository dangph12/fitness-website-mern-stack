...
const UpdatePlan = () => {
  ...
  useEffect(() => {
    if (currentPlan && currentPlan._id === id) {
      // Group workouts by days (assuming workouts are sequential)
      const workoutsPerDay = Math.ceil((currentPlan.workouts?.length || 0) / 3);
      const daysData = [];
      let workoutIndex = 0;

      // If no workouts, create one empty day
      if (!currentPlan.workouts || currentPlan.workouts.length === 0) {
        daysData.push({ dayName: 'Day 1', workouts: [] });
      } else {
        // Group workouts into days
        while (workoutIndex < currentPlan.workouts.length) {
          const dayWorkouts = currentPlan.workouts
            .slice(workoutIndex, workoutIndex + workoutsPerDay)
            .map(workout => ({
              _id: workout._id,
              title: workout.title,
              image: workout.image,
              exercises: (workout.exercises || []).map(ex => ({
                exercise: {
                  ...
                  title: ex.exercise?.title || 'Exercise',
                  tutorial: ex.exercise?.tutorial || ex.exercise?.image
                },
                sets: ex.sets || [12]
              }))
            }));
          ...
          daysData.push({
            dayName: `Day ${daysData.length + 1}`,
            workouts: dayWorkouts
          });

          workoutIndex += workoutsPerDay;
        }
      }
  ...
  const onSubmit = async data => {
    ...
    if (isImageChanged && data.image) {
      console.log('Adding new plan image');
    ...
    try {
      ...
      const result = await dispatch(
        updatePlan({ id, updateData: formData })
      ).unwrap();
