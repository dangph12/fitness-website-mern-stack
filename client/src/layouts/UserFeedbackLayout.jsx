...
function UserFeedbackLayout() {
  return (
    <div className="bg-gray-100 text-gray-900 min-h-screen p-35">
      <h1 className="text-4xl font-bold text-center mb-10 text-[#3067B6]">What Our Clients Say</h1>
      <p className="text-center mb-10 text-lg text-[#3067B6]">Hear from gym owners and fitness enthusiasts who trust our trainers to help them reach their fitness goals. Their feedback inspires us to continue delivering top-tier services.</p>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {feedbacks.map((feedback, index) => (
          <div key={index} className="bg-white p-6 rounded-xl shadow-xl hover:shadow-2xl transition-shadow duration-300 ease-in-out">
            <div className="flex items-center">
              <div className="w-16 h-16 bg-gray-300 rounded-full flex items-center justify-center mr-5 shadow-lg">
                {feedback.icon}
              </div>
              <div>
                <h3 className="text-xl font-semibold text-[#3067B6]">{feedback.name}</h3>
