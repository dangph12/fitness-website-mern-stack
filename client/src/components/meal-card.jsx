...
export default function MealCard({ meal }) {
  ...
  const foodsSlice = useSelector(state => state.foods.foods.foods || []);
  const [detailedFoods, setDetailedFoods] = useState([]);

  useEffect(() => {
    const fetchFoods = async () => {
      const promises = meal.foods.map(async f => {
        let foodInfo = foodsSlice.find(fd => fd._id === f.food);
        if (!foodInfo) {
          try {
            foodInfo = await dispatch(fetchFoodById(f.food)).unwrap();
          } catch {
            foodInfo = null;
          }
        }
      ...
      setDetailedFoods(results);
    };

    fetchFoods();
  }, [meal.foods, foodsSlice, dispatch, meal.scheduledAt]);
  ...
  return (
    <div className='bg-white rounded-2xl shadow-lg p-6 hover:shadow-2xl transition'>
      ...
      <div className='mt-4 flex gap-3 flex-wrap'>
        <span className='flex items-center gap-1 bg-red-200 text-red-700 px-3 py-1 rounded-full font-semibold'>
          <FaFire /> {totals.calories} kcal
        </span>
        <span className='flex items-center gap-1 bg-green-200 text-green-700 px-3 py-1 rounded-full font-semibold'>
          <FaDrumstickBite /> P: {totals.protein}g
        </span>
        <span className='flex items-center gap-1 bg-yellow-200 text-yellow-700 px-3 py-1 rounded-full font-semibold'>
          <FaAppleAlt /> C: {totals.carb}g
        </span>
        <span className='flex items-center gap-1 bg-blue-200 text-blue-700 px-3 py-1 rounded-full font-semibold'>
